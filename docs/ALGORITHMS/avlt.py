...
class BSTree:
    ...
    def AVL(self, root):
        # Retorna Factor de Balanceo

        #Caso 1: No tiene hijos ---> factor de balanceo = 0
        print(root.get_left())
        print(root.get_right())
        return (self.height(root.get_left())) - (self.height(root.get_right()))
...
def main():
    ...
    # INORDER
    ird = tree.inorder(root) # IMPRIME LA LISTA DEL RECORRIDO DEL ARBOL INORDER
    print(ird)

    # PREORDER

    prd = tree.preorder(root) # IMPRIME LA LISTA DEL RECORRIDO DEL ARBOL EN PREORDER
    print(prd)

    # POSTORDER
    prd = tree.postorder(root)
    print(prd)


    # SECCION AVL

    # nroot, at = tree.search(n) EStructura de definición del nodo a definir para su respectiva altura
    #nroot, at = tree.search(27)
    #fc = tree.AVL(nroot)
    #print(fc)
    ...
    """
    # EJEMPLO DE USO:
    nroot, at = tree.search(73)
    hg = tree.height(nroot)
    print(hg)
    """
main()